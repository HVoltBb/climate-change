...
Type objective_function<Type>::operator() ()
{
	...
	for(int i=0; i < l1.size(); i++){
		...
		if(ds < PAR4){
			...
			lt = grow::fish<Type>.growth_fn(lt, c + lsex(sx) + naoc_d(tb+j) + PAR5*(intrinsic_effect), k + ksex(sx) + naok_d(tb+j) + (1-PAR5)*intrinsic_effect, (Type) 1/30.0, PAR1, exp(lognu));
			}
		}else{
			tb = t1(i);
			td = t2(i) - t1(i);
			if(td == 0){		
				...
				lt = grow::fish<Type>.growth_fn(lt, c + lsex(sx) + naoc(tb) + PAR5*intrinsic_effect, k + ksex(sx) + naok(tb) + (1-PAR5)*intrinsic_effect, d2(i) - d1(i), PAR1, exp(lognu));
			} else {			
				// Released and recaptured in different months
				intrinsic_effect = indl(id) + theta_l(ms);
				lt = grow::fish<Type>.growth_fn(lt, c + lsex(sx) + naoc(tb) + PAR5*intrinsic_effect, k + ksex(sx) + naok(tb) + (1-PAR5)*intrinsic_effect, 1 - d1(i), PAR1, exp(lognu));
				for(int j = 1; j < td; j++){
					intrinsic_effect = indl(id) + theta_l((ms+j)%12);
					lt = grow::fish<Type>.growth_fn(lt, c + lsex(sx) + naoc(tb+j) + PAR5*intrinsic_effect, k + ksex(sx) + naok(tb+j) + (1-PAR5)*intrinsic_effect, days(tb+j), PAR1, exp(lognu));
				}
				// Second partial month
				intrinsic_effect = indl(id) + theta_l((ms+td)%12);
				lt = grow::fish<Type>.growth_fn(lt, c + lsex(sx) + naoc(t2(i)) + PAR5*intrinsic_effect, k + ksex(sx) + naok(t2(i)) + (1-PAR5)*intrinsic_effect, d2(i), PAR1, exp(lognu));
			}
		}
	...
	for(int i = 0; i < Pdim; i++){
		gam_l(i) = grow::fish<Type>.growth_fn(ref_len, c + lsex(ref_sex) + gam_c(i) + PAR5*intrinsic_effect, k + ksex(ref_sex) + gam_k(i) + (1-PAR5)*intrinsic_effect, ref_tim, PAR1, exp(lognu));
	}
	...
	for(int i = 0; i < days_p.size(); i++){
		tmp = grow::fish<Type>.growth_fn(ref_len, c + lsex(ref_sex) + gam_cp(i), k + ksex(ref_sex) + gam_kp(i), lag_/30., PAR1, exp(lognu));
		ref_p(i) = grow::fish<Type>.growth_fn(tmp, c + lsex(ref_sex) + gam_cp(i+1), k + ksex(ref_sex) + gam_kp(i+1), days_p(i) - lag_/30., PAR1, exp(lognu));
	}
